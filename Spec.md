...
RP2350A 搭載ボード + ClockworkPi PicoCalc 上で、「カエルのために鐘は鳴る」（DMG Game Boy）を動かすための専用エミュレータ。
...
PicoCalc 上で「カエルのために鐘は鳴る」を快適にプレイできる状態にする。
...
| キーボードバックライト | STM32 ファームウェアが 0x0A レジスタを未実装とみられる。メニューから削除済み（Flash 設定形式は維持） |
| FPS 表示 | 未実装 |
