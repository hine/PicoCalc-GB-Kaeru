...
| **色並び順** | **BGR**（MADCTL 0x48 bit3=1 で確認済み）→ LovyanGFX `rgb_order = false` 必須 |
...
```
y=0〜15  (16px) 上部ストリップ（16px）:
  x=0〜49    ステータステキスト（Font0 6×8px、8 文字、F キー操作メッセージ等）
  x=282〜305 バッテリー残量（4 文字: " 75%" 形式、色: 緑/黄/赤）
  x=310〜317 ストレージアイコン（8×12px: 白=読込・青=SD書込・黄=Flash書込）
...
- ✅ ILI9488 BGR 色並び順修正（`rgb_order = false`、実機確認済み 2026-05-25）
- 🔄 バッテリー残量表示（UI 実装済み・I2C レジスタ 0x0B 動作確認中）
- ⬜ メニュー UI（ROM リロード・セーブ削除・バックアップ）
