...
最終更新: 2026-05-25（F1スリープ撤去・ファンクションキー再割り当て）
...
- [x] キーバインド更新（WASD 追加・`[`/`]` AB ボタン・BS/Enter=Start・Del=Select、2026-05-25）
- [x] ILI9488 BGR 色並び順修正（`rgb_order=false`、全色正常表示、実機確認済み 2026-05-25）
- [x] バッテリー残量表示 UI（上部ストリップ右側、30秒更新、緑/黄/赤、実装済み 2026-05-25）
- [x] バッテリー I2C 0x0B 調査完了（STM32 ファームウェア未実装・NACK 確認。"--" 表示で運用。clockworkpi/PicoCalc#20 参照）
- [x] メニュー UI 実装完了（ESC 開く・B 閉じる、実機確認済み 2026-05-25）
  - パレット切替（DMGGreen/Mono/Sepia/GBPocket）リアルタイムプレビュー
  - 音声 ON/OFF トグル
  - SD バックアップ（完了トースト表示）
  - SD からFlash 復元（確認ダイアログ + 完了トースト）
  - Flash 全消去（確認ダイアログ）
  - 設定（パレット・音声）を Flash に保存・起動時ロード
- [x] ソフトリセット F1（GB 再初期化 + Flash SRAM リロード、実機確認済み 2026-05-25）
- [x] F1スリープ撤去（Flash ステートセーブ後に電源断で代替、dormant コード全削除 2026-05-25）
- [x] ファンクションキー再割り当て（F1=ソフトリセット / F2=ステートセーブ / F3=ステートロード / F4=スロット切替）
...
| 2026-05-25 | ESC でメニュー開く・B ボタンのみ閉じる | Enter/ESC 両方をメニュー開閉に使うと誤操作が多かった。ESC は開くのみ、閉じるは B ボタン（`]`/`.`）のみに分離。確認ダイアログのキャンセルも同様 |
| 2026-05-25 | メニュー中のゲームフレーム描画を g_menu_active フラグで抑制 | ESC ハンドラで `g_menu_active=true` → `g_lcd_busy=false` → `menu_open()` → `continue` の順にすることで、Core 1 がメニュー上にフレームを上書きする競合を解消 |
| 2026-05-25 | SD 操作の確認ダイアログとトーストを追加 | SD 復元は確認なしに上書きするリスクがあるため確認ダイアログを追加。バックアップ・復元完了後は緑色トーストで結果を表示し任意キーで閉じる |
| 2026-05-25 | F5 ソフトリセット追加（`gb_core_reset()` + Flash SRAM リロード） | ゲームのリセットを gb_reset() + APU 再初期化 + Flash SRAM リロードで実現。ROM の再読み込みや Core 1 の再起動は不要。cart_ram を再 malloc しないため安全 |
| 2026-05-25 | キーボードバックライト制御断念（メニューから削除） | STM32 ファームウェアが 0x0A レジスタの write を実装していない（または未対応バージョン）と判断。公式プロトコル（0x8A, val + sleep_ms(16) + readback）を試みたが反応なし。起動時の設定ロードと Flash 保存形式は維持し、対応ファームウェアに更新された場合に再追加可能 |
| 2026-05-25 | F1スリープ（dormant）撤去 | F2 ステートセーブ後に電源断すれば同等の効果が得られるため不要と判断。powman 依存コード・SLEEP_MAGIC/RESUME_MAGIC・go_dormant() を全削除。セーブステートスロットを 11→10 に縮小（スリープ専用スロット廃止）|
| 2026-05-25 | ファンクションキー再割り当て | F1=ソフトリセット / F2=ステートセーブ / F3=ステートロード / F4=スロット切替。旧 F1 スリープ・旧 F3 スロット・旧 F4 ロード・旧 F5 リセットから整理 |
...
| バッテリー I2C 0x0B | STM32 ファームウェアが 0x0B を未実装（NACK）。clockworkpi/PicoCalc#20 でも同様の既知問題。ファームウェア更新待ち。UI は "--" 表示で対応済み |
| キーボードバックライト 0x0A | 公式プロトコル（0x8A, val + sleep_ms(16) + readback）を実装したが実機で無反応。バッテリーと同様にファームウェア未対応の可能性。メニューから削除済み（Flash 設定保存形式は維持）|
