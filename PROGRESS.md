...
すべてのマイルストーンが完了。現在は保留事項（バッテリー残量・キーボードバックライト）の解決待ち。
...
- [x] キーボードバックライト制御断念（STM32 ファームウェア未実装。メニューから削除済み）
...
| キーボードバックライト 0x0A | 公式プロトコルを実装したが実機で無反応。ファームウェア未対応の可能性。メニューから削除済み（Flash 設定保存形式は維持） |
...
| 2026-05-25 | キーボードバックライト制御断念 | STM32 ファームウェアが 0x0A レジスタを未実装とみられる。メニューから削除 |
